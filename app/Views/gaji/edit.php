...
<div class="container">
    <div class="col">
        <h3 class="text-center">Edit Data gaji</h3>
        <br>
        <form action="/gaji/update/<?= $gaji['id']; ?>" method="post">
            <?= csrf_field(); ?>
                ...
                <div class="col-sm-6">
                    ...
                    <select class="form-select <?= ($validation->hasError('nip')) ? 'is-invalid' : ''; ?>" id="nip" name="nip">
                        ...
                        <?php foreach ($karyawan as $row): ?>
                            <option value="<?= $row['nip']; ?>" <?= ($gaji['nip'] == $row['nip']) ? 'selected' : ''; ?>>
                                <?= $row['nip']; ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="col-sm-6">
                    ...
                    <select class="form-select <?= ($validation->hasError('kd_bagian')) ? 'is-invalid' : ''; ?>" id="kd_bagian" name="kd_bagian">
                        ...
                        <?php foreach ($bagian as $row): ?>
                            <option value="<?= $row['kd_bagian']; ?>" <?= ($gaji['kd_bagian'] == $row['kd_bagian']) ? 'selected' : ''; ?>>
                                <?= $row['kd_bagian']; ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-6">
                    <input type="text" class="form-control" id="gaji_bersih" name="gaji_bersih"
                        value="<?= old('gaji_bersih') ?? $gaji['gaji_bersih']; ?>">
                </div>
            </div>
            <br>
            <!-- Tambah field lainnya sesuai kebutuhan -->
            <button type="submit" class="btn btn-primary" style="width: 66%;">Update Data</button>
        </form>
        <br><br>
        <p class="card-text">
